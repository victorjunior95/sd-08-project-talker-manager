...
const validationAndRegexToken = (req, res, next) => {
  ...
  const nameAndAgeValidation = (req, res, next) => {
    ...
    if (name.length < 3) {
      return res.status(400).send({ message: M.REQUIRED_NAME_LENGTH });
    }
    if (!age) return res.status(400).send({ message: M.REQUIRED_FIELD_AGE });
    if (age < 18) {
      return res
        .status(400)
        .send({ message: M.CONDITION_AGE });
    }
    next();
  };

  const talkInfoValidation = (req, res, next) => {
    const {
    ...
    if (!Number.isInteger(rate) || rate < 1 || rate > 5) {
      return res
        .status(400)
        .send({ message: M.CONDITION_RATE });
    }
    const REGEX_DATA = /^([0-2][0-9]|(3)[0-1])(\/)(((0)[0-9])|((1)[0-2]))(\/)\d{4}$/i;
    if (!REGEX_DATA.test(watchedAt)) {
      return res
        .status(400)
        .send({ message: M.FORMATATION_DATE });
    }
    next();
  };
